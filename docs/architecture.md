...
PoC в репо доказывает инвариант политики: серая зона и stale cache **не** открывают турникет, причина видна в audit. Модели — mock/fixture; целевой стек — в [ml.md](ml.md).

Рядом уже лежит **prod-shaped** обвязка edge (всё ещё не прод): симулятор турникета с ack и защитой от двойного open, локальный revoke шаблона (delta policy_version), очередь охраны `/v1/guard/*`, счётчики `/metrics`. Это мост к железу и SoT, не замена реальных CV-моделей.
